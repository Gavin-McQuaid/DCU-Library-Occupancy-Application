<?php include '../inc/dbinfo.inc';
  ...
  $time = time();
  $query = "SELECT * FROM account_authentication  WHERE email = '$email' AND authentication_code = '$code' AND ('$time' - timestamp) <= 300";
  $doCheck = mysqli_fetch_array(mysqli_query($connection,$query));
  if(isset($doCheck)){
        $confirmed = 1;
        $accountConfirmedQuery = "UPDATE users SET account_confirmed='$confirmed'  WHERE email = '$email'";
        if(mysqli_query($connection,$accountConfirmedQuery)){
